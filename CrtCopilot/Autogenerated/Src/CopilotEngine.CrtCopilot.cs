	...
	internal class CopilotEngine : ICopilotEngine
	{
		...
		private CopilotChatPart SendSession(CopilotSession session) {
			lock (session) {
				...
				return new CopilotChatPart(messages, session, string.Empty, string.Empty);
			}
		}
		...
		public CopilotChatPart SendUserMessage(string content, Guid? copilotSessionId = null,
			...
			try {
				...
				var (errorMessage, errorCode) = GetErrorInfo(e);
				_log.Error(e);
				return new CopilotChatPart(new List<CopilotMessage> { session.Messages.Last() }, session, errorMessage, errorCode);
			}
		}
